...
\begin{description}
\item[data] CIT coreset expression data matrix
\item[data.cl] CIT breast cancer molecular subtypes of the coreset
\item[data.annot] probesets annotations provided by NetAffx (version na30) in order to map samples from other plateforms than Affymetrix
\end{description}


To assign a new sample to a CIT subtype ( \textbf{\textit{cit.assignBcmst}} function) the following steps are needed:
\begin{enumerate}
...
\item[citbcmst.mixed] subtypes of the closest centroids for each sample (for "mixed" samples)
\item[citbcmst.core] subtype of the closes sample for "core" samples ("outlier" and  "mixed" samples are set to NA)
